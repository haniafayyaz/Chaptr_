// controllers/readingListController.js
...
const removeFromReadingList = async (req, res) => {
  try {
    ...
    const book = await ReadingList.findOneAndDelete({ username, bookId });
    if (!book) {
      return res.status(404).json({ message: 'Book not found in reading list' });
    }
    res.json({ message: 'Book removed successfully', bookId });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
