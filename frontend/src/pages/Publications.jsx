...
const Publications = () => {
  ...
  useEffect(() => {
    const fetchAuthorProfile = async () => {
      try {
        ...
        setFilteredItems(data.authorProfile?.bookDetails || []); // Default to bookDetails for My Books
        setLoading(false);
      } catch (err) {
 ...
 cathedral                    <textarea
            ...
            {filteredItems.length === 0 ? (
              <p>No {activeTab === "myBooks" ? "books" : activeTab === "announcements" ? "announcements" : "release dates"} found.</p>
            ) : (
